...
| `SNAPSHOT_LIVE`                    | `auto`                          | policy for bare `/snapshot/<sn>` requests. `auto` uses the bridge's battery-capability policy (no live burst for battery-capable cameras); `1` forces a live burst for every camera; `0` disables live bursts. An individual request may override it with `?mode=auto`, `?mode=stored`, or `?mode=live`; the override applies only to that request and does not change `SNAPSHOT_LIVE`. If live acquisition is not attempted or fails, retained/persisted imagery is served when available |
